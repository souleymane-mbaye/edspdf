...
class TrainableClassifier(TrainablePipe[Dict[str, Any]]):
    ...
    def __init__(
        ...
        name: str = "type-trainable-classifier",
    ):
        ...
        self.classifier = torch.nn.Linear(
            ...
            # out_features=5, # {'pollution', 'title', 'front', 'table', 'p'}
        )
            ...
            **self.preprocess(doc),
            "labels": [
                [
                    ...
                    # {'pollution', 'title', 'front', 'table', 'p'}
                    # [
                    #     1.0 if b.label=="front" else 0.0,      # front label
                    #     1.0 if b.label=="title" else 0.0,      # front label
                    #     1.0 if b.label=="p" else 0.0,          # paragraphe label
                    #     1.0 if b.label=="table" else 0.0,      # table label
                    #     1.0 if b.label=="pollution" else 0.0,  # pollution label
                    # ]
            ...
            collated.update(
                {
                    "labels": as_folded_tensor(
                        ...
                        # dtype=torch.float,  # <- float, pour cross entropy (int)
    ...
    def forward(self, batch: Dict) -> Dict:
        ...
        if "labels" in batch:
            ...
            #
            # print(f'type logits: {logits.dtype} {logits.shape} -- targets type: {targets.dtype} {targets.shape}')
            output["label_loss"] = (
                # sig + BCELoss
    ...
    def postprocess(self, docs: Sequence[PDFDoc], output: Dict) -> Sequence[PDFDoc]:
        for b, label in zip(
            ...
            # {'pollution', 'title', 'front', 'table', 'p'}
            # b.label = 'front' if label == 0 else \
            #             ('title' if label == 1 else \
            #             ('p' if label == 2 else \
            #             ('table' if label == 3 else 'pollution')))
        return docs
