...
def _partition_lines_in_boxes_on_page(
    ...
    for box_lines in boxes_lines:
        ...
        if nb_lines > 1:
            box_lines.lines[0].bioul = 'B' # Beggin
            box_lines.lines[-1].bioul = 'L' # Last
            for li in box_lines.lines[1:-1]:
                li.bioul = 'I'
        elif nb_lines == 1:
            box_lines.lines[0].bioul = 'U' # Unit
    
    for box_lines in boxes_lines:
        # box_lines.lines = sorted(box_lines.lines, key=attrgetter('y0', 'x0'))
        if box_lines.box.label in ['text', 'title', 'pollution']:
            box_lines.text = box_lines.box.label + '@@:\n' + ' '.join([tb.text for tb in box_lines.lines])
        
        if box_lines.box.label == 'table':
            box_lines.text = box_lines.box.label + ':\n' + ' '.join([tb.text for tb in box_lines.lines])
            
            # ok = False
            # iprec = 0
            # prec = box_lines.lines[iprec]
            # while not ok:
            #     isuiv = iprec + 1
            #     suiv = box_lines.lines[isuiv]
                
            #     # case 1
            #     prec.x0 <= suiv.x0 and suiv
        
        if box_lines.box.label == 'list':
            box_lines.text = box_lines.box.label + ':\n' + '\n'.join([('--> '+tb.text) for tb in box_lines.lines])
...
def partition_lines_in_boxes(
    ...
    return [
        ...
        for b in _partition_lines_in_boxes_on_page(
            src_boxes=[
                b
                for b in src_boxes
