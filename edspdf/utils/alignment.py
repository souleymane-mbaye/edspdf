...
# align box labels ([B,L] or type{title,p,front,table}) with pkl
def _align_box_labels_bl_pkl_on_page(
    src_boxes: Sequence[Box],
    dst_boxes: Sequence[Box],
    threshold: float = 0.0001,
    pollution_label: Any = None,
...
def align_box_labels_bl_pkl(
    ...
    return [
        b
        for page in sorted(set((b.page_num for b in dst_boxes)))
        for b in _align_box_labels_bl_pkl_on_page(
            src_boxes=[
                b
                for b in src_boxes
                if page is None or b.page_num is None or b.page_num == page
...
# align box labels (order between lines of same node type, wether a line_i comes before or after line_j) with pkl
def _align_box_labels_order_ba_pkl_on_page(
    src_boxes: Sequence[Box],
    dst_boxes: Sequence[Box],
    threshold: float = 0.0001,
    pollution_label: Any = None,
    ...
    for i,b in enumerate(dst_boxes):
        ...
        if src_i < len(src_boxes):
            ...
            label = f'{pollution_label}{_sep}-1{_sep}-1'
            # b.label = pollution_label
            # b.node_num = -1
            # b.rank = -1
        b.label = label
        new_dst_boxes.append(b)
        
    
    return new_dst_boxes


def align_box_labels_order_ba_pkl(
    src_boxes: Sequence[Box],
    dst_boxes: Sequence[T],
    threshold: float = 0.0001,
    pollution_label: Any = None,
    ...
    return [
        b
        for page in sorted(set((b.page_num for b in dst_boxes)))
        for b in _align_box_labels_order_ba_pkl_on_page(
            src_boxes=[
                b
                for b in src_boxes
                if page is None or b.page_num is None or b.page_num == page
